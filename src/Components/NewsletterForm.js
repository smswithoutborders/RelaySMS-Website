...
const KEILA_SUBMIT_URL = "https://newsletter.afkanerd.com/forms/nfrm_BzLMaLXv";
const HONEYPOT_NAME = "h[url]";
...
const NewsletterForm = ({
	...
	const handleSubmit = async (event) => {
		...
		const honeypotValue = String(formData.get(HONEYPOT_NAME) ?? "").trim();
		...
		try {
			...
			payload.append(HONEYPOT_NAME, "");

			await fetch(KEILA_SUBMIT_URL, {
				method: "POST",
				mode: "no-cors",
				headers: {
			...
			{showForm ? (
				...
				>
					...
					>
						...
						>
							<input type="text" name={HONEYPOT_NAME} autoComplete="off" tabIndex={-1} defaultValue="" />
						</Box>
